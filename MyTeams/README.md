...
MyTeams est un projet réseau Epitech (NWP) visant à recréer un outil de communication similaire à Microsoft Teams.
Il repose sur une architecture client/serveur asynchrone utilisant les sockets réseau en C++ (géré par `poll`).
...
```bash
# Compiler le projet complet (Serveur et Client)
make
...
Les exécutables générés seront situés à la racine du projet :
- `myteams_server`
- `myteams_cli`
...
./myteams_server <port>
```
- `<port>` : Le port d'écoute sur lequel le serveur va accepter les connexions TCP entrantes.

### Lancer le Client
```bash
./myteams_cli <ip> <port>
```
- `<ip>` : L'adresse IP du serveur auquel se connecter (ex: `127.0.0.1`).
- `<port>` : Le port d'écoute du serveur.


## 📜 Norme RFC - Protocole MyTeams

*(Ceci est le protocole de communication réseau custom implémenté entre le client et le serveur)*
